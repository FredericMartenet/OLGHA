...
def household_ss_olg(params, w, gov_ss, pop):
    """
    Solves the household's problem for a given real wage, interest rate, and social security policy.
    """
    # Construct labor income
    y_js = (
        (1 - gov_ss.tau) * w * params['y_eps'][np.newaxis, :] * pop.h[:, np.newaxis]
        + gov_ss.d * pop.iret[:, np.newaxis]
    )

    # allocate arrays to store results
    uc, c, a, D = (np.empty((pop.T + 1, params['N_eps'], params['N_a'])) for _ in range(4))
    ...
    for j in reversed(range(pop.Tw, pop.T+1)):
        if j == pop.T:
            ...
            coh_T = get_coh(y_js[j, ], params['r'], pop.phi[j], params['a'])
        ...
        # Compute cash-on-hand and bequest factor
        coh = get_coh(y_js[j-1, ], params['r'], pop.phi[j-1], params['a'])

        # Call backward iteration function
        a[j-1, ], c[j-1, ], uc[j-1, ] = backward_iterate_olg(
            uc[j, ], params['a'], params['Pi_eps'], coh, params['r'], params['beta'], params['sigma'])
    ...
    for j in range(pop.Tw, pop.T):
        ...
        D[j+1, ] = forward_step(dst_temp, Pi_T, a_pol_i, a_pol_pi)

    # Assets
    A_j = np.einsum('jsa,jsa->j', D, a)  # by age j
    A = np.einsum('j,j', pop.pi, A_j)  # Aggregate assets

    # Consumption
    C_j = np.einsum('jsa,jsa->j', D, c)  # by age j
    C = np.einsum('j,j', pop.pi, C_j)  # Aggregate consumption
